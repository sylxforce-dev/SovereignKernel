...
- **`CMAKE_CUDA_ARCHITECTURES native` targets only the GPU in the machine that compiles it** (currently an RTX 5060 Ti). The resulting binary won't run on a different GPU generation — recompiling on the target machine, or setting an explicit architecture list, is required.
- **Model and tokenizer file paths are hardcoded** as absolute Windows paths inside `main()` in both `real_runner.cpp` and `real_runner2.cpp` (e.g. `C:/Users/DrSulxX/CLionProjects/SovereignKernel/model_data/...`). You'll need to point these at wherever you've placed the `.bin`/GGUF weights and tokenizer file on your own machine.
