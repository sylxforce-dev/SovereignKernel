...
// =============================================================================
// [T4 EXPERIMENT] PROMPT LOOKUP DECODING N-GRAM OTSING
// =============================================================================
std::vector<int> find_ngram_match(const std::vector<int>& history, int match_len = 3, int max_candidates = 4) {
    if (history.size() < static_cast<size_t>(match_len + 1)) return {};
    std::vector<int> target(history.end() - match_len, history.end());

    // Otsime ajaloost tagurpidi kõige värskemat N-gram kattuvust
    for (int i = static_cast<int>(history.size()) - match_len - 1; i >= 0; --i) {
        bool match = true;
        for (int j = 0; j < match_len; ++j) {
            if (history[i + j] != target[j]) {
                match = false;
                break;
            }
        }
        if (match) {
            ...
            for (int j = 0; j < max_candidates && (i + match_len + j) < static_cast<int>(history.size()); ++j) {
                candidates.push_back(history[i + match_len + j]);
            }
...
__global__ void rmsnorm_kernel_float4(const float* __restrict__ x, const float* __restrict__ weight, float* __restrict__ out, int n, float eps) {
    ...
    for (int i = tid; i < n4; i += block_size) {
        ...
        float4 res; res.x = v.x * inv_rms * w.x; res.y = v.y * inv_rms * w.y;
        res.z = v.z * inv_rms * w.z; res.w = v.w * inv_rms * w.w; out4[i] = res;
    }
}

__global__ void add_and_rmsnorm_kernel_float4(float* __restrict__ x, const float* __restrict__ residual, const float* __restrict__ weight, float* __restrict__ out, int n, float eps) {
    extern __shared__ float sdata[];
    int tid = threadIdx.x; int block_size = blockDim.x; int n4 = n / 4;
    float partial = 0.0f;
    ...
    for (int i = tid; i < n4; i += block_size) {
        ...
        float4 res; res.x = vx.x * inv_rms * w.x; res.y = vx.y * inv_rms * w.y;
        res.z = vx.z * inv_rms * w.z; res.w = vx.w * inv_rms * w.w; out4[i] = res;
    }
}
...
// 🔥🔥 THE BEAST V3: MLP Unrolled Memory Loads + Online Softmax
...
__global__ void rope_q_batched_kernel(float* q, int num_heads, int head_dim, int N, float base, int pos_offset) {
    ...
    if (token_idx < N && idx < num_heads * half_dim) {
        ...
        float val = static_cast<float>(pos_offset + token_idx) * freq; float cos_val = cosf(val), sin_val = sinf(val);
...
__global__ void rope_k_inplace_batched_kernel(float* k, int kv_heads, int head_dim, int kv_dim, int N, float base, int pos_offset) {
    int token_idx = blockIdx.y; int idx = blockIdx.x * blockDim.x + threadIdx.x; int half_dim = head_dim / 2;
...
__global__ void float2half_copy_batched_kernel(const float* __restrict__ in, half* __restrict__ out, int stride, int N, int pos_offset) {
    ...
    if (token_idx < N && idx < stride / 2) {
        ...
        // Kirjutab õigesse cache asukohta absoluutse pos_offset abil
        reinterpret_cast<half2*>(out + (pos_offset + token_idx) * stride)[idx] = __floats2half2_rn(v0, v1);
    }
}
...
// =============================================================================
// TACTICAL ENGINE 1: CUDA GRAPHS (Puhas V1 GEMV - Max Kiirus N=1)
// =============================================================================
void forward_pass_decode_cuda(const CUDATransformerModelQ4& model, CUDABuffersQ4& bufs) {
    ...
    for (int l = 0; l < cfg.num_layers; ++l) {
        ...
        rmsnorm_kernel_float4<<<1, 256, norm_smem, bufs.stream>>>(bufs.x, lw.attention_norm, bufs.xb, dim, 1e-5f);
        ...
        add_and_rmsnorm_kernel_float4<<<1, 256, norm_smem, bufs.stream>>>(bufs.x, bufs.attn_out, lw.ffn_norm, bufs.ffn_norm, dim, 1e-5f);
    ...
    rmsnorm_kernel_float4<<<1, 256, norm_smem, bufs.stream>>>(bufs.x, model.final_norm, bufs.final_hidden, dim, 1e-5f);
...
// =============================================================================
// TACTICAL ENGINE 2: T4 SPECULATIVE BATCH (WMMA TENSOR CORES N>1 JAOKS)
// =============================================================================
void forward_pass_batched_speculative(const CUDATransformerModelQ4& model, const std::vector<int>& tokens, int pos_offset, CUDABuffersQ4& bufs, float* host_logits_multi) {
    ...
    for (int l = 0; l < cfg.num_layers; ++l) {
        ...
        tensor_math_quantized_wmma::launch_gemm_q4_0_wmma(lw.wq, bufs.xb_fp16, bufs.q, dim, N, dim, bufs.stream);
        tensor_math_quantized_wmma::launch_gemm_q4_0_wmma(lw.wk, bufs.xb_fp16, bufs.k, kv_dim, N, dim, bufs.stream);
        tensor_math_quantized_wmma::launch_gemm_q4_0_wmma(lw.wv, bufs.xb_fp16, bufs.v, kv_dim, N, dim, bufs.stream);

        dim3 grid_rq((cfg.num_heads * (head_dim / 2) + 255) / 256, N);
        rope_q_batched_kernel<<<grid_rq, 256, 0, bufs.stream>>>(bufs.q, cfg.num_heads, head_dim, N, 10000.0f, pos_offset);

        dim3 grid_rk((cfg.num_kv_heads * (head_dim / 2) + 255) / 256, N);
        rope_k_inplace_batched_kernel<<<grid_rk, 256, 0, bufs.stream>>>(bufs.k, cfg.num_kv_heads, head_dim, kv_dim, N, 10000.0f, pos_offset);
        ...
        float2half_copy_batched_kernel<<<grid_kv, 256, 0, bufs.stream>>>(bufs.k, bufs.k_cache[l], kv_dim, N, pos_offset);
        float2half_copy_batched_kernel<<<grid_kv, 256, 0, bufs.stream>>>(bufs.v, bufs.v_cache[l], kv_dim, N, pos_offset);
    ...
    dim3 grid_norm(1, N);
    rmsnorm_batched_kernel_float4<<<grid_norm, 256, norm_smem, bufs.stream>>>(bufs.x, model.final_norm, bufs.final_hidden, dim, N, 1e-5f);

    // Tagastame logits KÕIKIDE tokenite jaoks (N * vocab_size)
    for (int i = 0; i < N; ++i) {
        gemv_f32_kernel_warp_float4<<< (cfg.vocab_size + 3) / 4, 128, 0, bufs.stream>>>(model.output_weights, bufs.final_hidden + i * dim, bufs.logits + i * cfg.vocab_size, cfg.vocab_size, dim);
    }

    cudaStreamSynchronize(bufs.stream);
    cudaMemcpyAsync(host_logits_multi, bufs.logits, N * cfg.vocab_size * sizeof(float), cudaMemcpyDeviceToHost, bufs.stream);
    cudaStreamSynchronize(bufs.stream);
}
...
int main() {
    ...
    std::cout << "=== Sovereign Kernel: DUAL ENGINE (T4 Speculative Decoder) ===\n";
    try {
        ...
        std::vector<float> host_logits_multi(config.seq_len * config.vocab_size);
        std::random_device rd; std::mt19937 rng(rd());

        std::cout << "\n=======================================================\n";
        std::cout << " T4 SPECULATIVE INFERENCE READY (Dynamic WMMA injection)\n";
        std::cout << "=======================================================\n\n";
        ...
        while (true) {
            ...
            std::vector<int> full_sequence = tokens;

            std::cout << "\n[Misha Hybrid Output]: " << std::flush;
            bufs.reset_kv_cache(config);
            int pos = 0;

            // --- TAASTATUD: Prefill latentsuse mõõtmine ---
            auto prefill_start = std::chrono::high_resolution_clock::now();
            forward_pass_batched_speculative(cuda_model, tokens, pos, bufs, host_logits_multi.data());
            memcpy(host_logits.data(), host_logits_multi.data() + (tokens.size() - 1) * config.vocab_size, config.vocab_size * sizeof(float));
            pos += tokens.size();
            ...
            for (int step = 0; step < rcfg.max_tokens; ) {
                ...
                generated_token_count++; step++;

                if (rolling_check_buffer.find(stop_sequence) != std::string::npos) break;

                std::vector<int> draft = find_ngram_match(full_sequence, 3, 4);

                if (draft.empty()) {
                    // --- TAASTATUD: N=1 Tõelise Forward Pass aja mõõtmine ---
                    ...
                    std::vector<int> batch = {next_token};
                    batch.insert(batch.end(), draft.begin(), draft.end());
                    forward_pass_batched_speculative(cuda_model, batch, pos, bufs, host_logits_multi.data());
                    cudaStreamSynchronize(bufs.stream);
                    ...
                    for (size_t i = 0; i < batch.size(); i++) {
                        if (i == batch.size() - 1) {
                            memcpy(host_logits.data(), host_logits_multi.data() + i * config.vocab_size, config.vocab_size * sizeof(float));
                            pos++;
                            break;
                        }

                        int spec_sampled = sample_token(host_logits_multi.data() + i * config.vocab_size, config.vocab_size, rcfg.temperature, rcfg.top_p, rcfg.rep_penalty, recent_tokens, rng);

                        if (spec_sampled == draft[i]) {
                            spec_hits++;
                            full_sequence.push_back(spec_sampled);
                            recent_tokens.push_back(spec_sampled);
                            if (recent_tokens.size() > static_cast<size_t>(rcfg.penalty_window)) recent_tokens.erase(recent_tokens.begin());

                            std::string spec_piece = tokenizer.decode(spec_sampled);
                            rolling_check_buffer += spec_piece;
                            std::cout << spec_piece << std::flush;
                            generated_token_count++; step++;
                            pos++;
                        } else {
                            memcpy(host_logits.data(), host_logits_multi.data() + i * config.vocab_size, config.vocab_size * sizeof(float));
                ...
                // --- TAASTATUD: Dünaamiline 20-tokeni telemeetria aken ---
                if (generated_token_count - last_window_token_count >= 20) {
                    ...
                    int tokens_in_window = generated_token_count - last_window_token_count;
                    double window_ms_per_token = window_time.count() / tokens_in_window;
                    double window_tok_sec = 1000.0 / window_ms_per_token;

                    std::cout << "\n  [Telemetry] Tokens " << last_window_token_count + 1 << "-" << generated_token_count
                              << " | Speed: " << window_tok_sec << " tok/s | Avg: " << window_ms_per_token << " ms/token\n";

                    window_start = std::chrono::high_resolution_clock::now();
                    last_window_token_count = generated_token_count;
                }
            }
            ...
            // --- TAASTATUD: Detailne finaalraport ---
            std::cout << "\n\n[CUDA Performance Stats] Generated: " << generated_token_count
                      ...
                      << " | Prefill Latency: " << prefill_ms.count() << " ms | Speculative Free Tokens: " << spec_hits << "\n";
        }
